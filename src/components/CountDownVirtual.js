...
class CountDown extends React.Component {
  ...
  renderDoubleDigits = (label, digits) => {
    ...
    if(isCircle){
      ...
      return (
        <View key={label} style={{justifyContent: 'center', alignItems: 'center', backgroundColor:'white', width:80, height:80, margin:5, borderRadius: 40, borderWidth:5, borderColor:lineColor}}>
      ...
      return (
        <View key={label} style={{backgroundColor:'white'}}>
          <View>
            <Text style={{fontSize:30}}>{digits}{label != 'Detik' ? ':' : ''}</Text>
          </View>
        </View>
      )
    }
