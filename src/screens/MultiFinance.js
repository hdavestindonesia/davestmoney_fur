...
export default class extends Component {
	...
	onProses = () => {
		...
		// Alert.alert("idPelanggan",idPelanggan)
		// Alert.alert("produkSelected",produkSelected)



		if(produkSelected != "" && idPelanggan != "")
		{

			generateCounter((counter) => {

				// Alert.alert("user",JSON.stringify(user))
				// Alert.alert("produkSelected",JSON.stringify(produkSelected))
				// Alert.alert("idPelanggan",JSON.stringify(idPelanggan))
				...
				postXML('', xml, true, (resp) => {
					...
					if(resp.posh.resultcode[0] == '0000'){
						...
						this.props.navigation.navigate("KonfirmasiMultiFinance",{
							data: data,
							user
						})
	...
	render() {
		let {user, idPelanggan, produkSelected, pembayaran, optionsMultifinance} = this.state

		// Alert.alert("optionsMultifinance",JSON.stringify(optionsMultifinance))

		if(!user){
			return null
		}

		if(!optionsMultifinance)
		{
			return (
		...
		if(optionsMultifinance)
		{
			return (
				<ScrollView style={{backgroundColor:'white', padding:20}}>
					...
					<View>
						<Text style={{color:'black'}}>Produk Multifinance</Text>
						...
						>
						  ...
						  { optionsMultifinance.map((item, key)=>(
